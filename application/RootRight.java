...
/**
 * This is the class of the right part of the root of primary GUI, it implements
 * the meal list, its related functions, and load and save file function of our
 * program
 * 
 * @author shifan, etc
 *
...
public class RootRight {
	...
	/**
	 * This is the constructor to help us access to the FoodData data created in
	 * main
	 * 
	 * @param foodData
	 */
	...
	public static void start(BorderPane root) {
		try {
			...
			// Set the relationship of each vbox and hbox, and the background color, style
			// of the largest
			// box
			vbox.getChildren().addAll(mealList, hbox1, hbox2);
			vbox.setSpacing(5);
			vbox.setStyle("-fx-background-color: #8FAADC;" + "-fx-border-color: #000000;" + "-fx-padding: 10;"
					+ "-fx-border-color: #000000;" + "-fx-border-width: 3;" + "-fx-border-style: solid inside;");
			...
			// pop up a new window with Meal Analysis represented by pie chart when analyze
			// button is
			// clicked
			...
			// Use FileChooser to choose a new file to open and load Food data in the file
			// to our food
			...
			loadButton.setOnAction(e -> {
				...
				// load file content into our food data instance, sort it, and show on the food
				// list
				if (file != null) {
			...
			// the saveButton is used to create a file chooser and save the file in the
			// project folder
	...
	/**
	 * the method that analyzes the foodItems in the meal list and generate a pie
	 * chart to display the analysis results.
	 */
	private static void analyze() {
		...
		ObservableList<PieChart.Data> pieChartData = FXCollections.observableArrayList(
				...
				new PieChart.Data("Carbohydrate", totalCarbo), 
				new PieChart.Data("Fiber", totalFiber),
				new PieChart.Data("Protein", totalProtein)
				);
		pieChartData.forEach(
				data -> data.nameProperty().bind(Bindings.concat(data.getName(), " ", 
